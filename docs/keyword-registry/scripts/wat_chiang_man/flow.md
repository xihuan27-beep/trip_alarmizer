...
**타 사이트 연결(2025-09 추가, 2025-09 수정 — 순서 가정 금지)**: 화자(실존 인물)가 다른 사이트에서도 비중 있게 등장한다면, 스크립트 맨 끝에 그 사이트를 언급하며 마무리한다. `site_links` 테이블에서 같은 keyword_id(인물)가 걸린 다른 site_id를 확인해 고른다. 이 사이트는 `khruba_srivichai_phra_singh_abbot`/`khruba_srivichai_tiger_order`/`khruba_srivichai_1935_disrobing_crisis` 등이 전부 `wat_phra_singh`에도 걸려 있고, 그가 사실상 주지로 지낸 핵심 거점이므로 왓 프라싱을 언급.

**중요**: 사용자가 사이트를 어떤 순서로 방문할지 전혀 알 수 없다. "다시 만났네요", "지난번에 약속드린 대로", "이어서 들려드릴게요", "마지막 부분" 같이 **다른 사이트를 먼저/나중에 들었다는 걸 전제하는 표현은 절대 쓰지 않는다.** 각 사이트 스크립트는 다른 어떤 사이트도 안 들은 사람이 들어도 완전히 이해되는 독립된 글이어야 한다. 교차 언급은 항상 "~에 가시면 제 얘기를 더 들으실 수 있어요"처럼, 순서·선후 관계를 내포하지 않는 순수 정보성 포인터로만 쓴다. 화자의 자기소개도 사이트마다 매번 처음 만나는 사람에게 하듯 완전하게 반복한다.
